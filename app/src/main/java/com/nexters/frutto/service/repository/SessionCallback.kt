...
class SessionCallback(application: Application) : ISessionCallback {
    ...
    init {
        ...
        if (LoginActivity.sf!!.contains("kakao_nickname"))
            user = KakaoUser(
                LoginActivity.sf!!.getString("kakao_id", "null"),
                LoginActivity.sf!!.getString("kakao_nickname", "null"),
                LoginActivity.sf!!.getString("kakao_token", "null")
            )
        else {
    ...
    override fun onSessionOpenFailed(exception: KakaoException?) = Unit
